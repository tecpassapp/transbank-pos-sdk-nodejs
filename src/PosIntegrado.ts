...
export class POSIntegrado extends POSBase {
	async closeDay(): Promise<CloseDayResponse> {
		const data = await this.send('0500||');
	...
	async getLastSale(): Promise<SaleResponse> {
		const data = await this.send('0250|');
		try {
	...
	async getTotals(): Promise<GetTotalsResponse> {
		const data = await this.send('0700||');
		const chunks = (data as string).split('|');
		return {
			functionCode: parseInt(chunks[0] ?? '0'),
			responseCode: parseInt(chunks[1] ?? '0'),
