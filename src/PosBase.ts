...
export class POSBase extends EventEmitter {
	...
	send(
		...
		return new Promise((resolve, reject) => {
			if (!this.connected) {
				...
				const error = new Error('You have to connect to a POS to send this message: ' + payloadString);
				return reject(error);
			}
			...
			// Wait for the response and fulfill the Promise
	...
	async loadKeys(): Promise<LoadKeysResponse> {
		const data = await this.send('0800');
